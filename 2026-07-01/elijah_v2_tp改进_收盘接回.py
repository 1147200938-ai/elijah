        ...
        if in_pos:
            ...
            if exit_reason:
                ...
                if i + 1 < len(data):
                    entry_price = data[i+1]['o']
                    entry_idx = i + 1; entry_bar = prev
                    in_pos = True; pos_type = reverse_type
            ...
            # === 新TP逻辑：每根K线触及13%就止盈+收盘价接回 ===
            if exit_reason is None:
                tp_hit = False; tp_price = None
                if pos_type == 'long':
                ...
                if tp_hit:
                    ...
                    trades[-1]['exit_time'] = cur['dt']
                    trades[-1]['pnl_pct'] = round(pnl, 2)
                    trades[-1]['reason'] = f'止盈(+13%)'
                    print(f"    止盈 idx={i} at {cur['dt']} 价={tp_price:.8f} pnl={pnl:+.2f}% -> 收盘接回 {cur['c']:.8f}")
                    # 当前K线收盘接回，继续持仓
                    re_entry = cur['c']
                    entry_price = re_entry
                    entry_idx = i
                    entry_bar = cur
                    trades.append({
                        'type':f's2_{pos_type}_re',
                        'entry': re_entry,
                        'exit': None,
                        'entry_time': cur['dt'] + '(close)',
                        'exit_time': None,
                        'pnl_pct': 0,
                        'reason': None
                    })
